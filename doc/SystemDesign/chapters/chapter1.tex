...
This document will be maintained by the entire working group, with changes made as they see fit. Changes to the plan after version 1.0 may be made by any individual person of the team, but only after discussing them with the other team members.
...
The CCSDS 132 encoder forms part of the space component of CCSDS 132.0-B-3. In its current configuration, it provides higher system levels with the packet service, as described in CCSDS 132.0-B-3. The standard distinguishes between physical, master, and virtual channels. Each CCSDS-132 implementation provides one physical channel, which describes the hardware transmission to the lower layers. The master and virtual channels are for organising the internal data structure and do not affect how data is transmitted, except for the virtual and master channel IDs inside the TM Transfer Frame header, over the physical channel. According to this definition, the CCSDS 132 encoder must prioritise and multiplex incoming virtual and master channel combinations in order to enable continuous data transmission. Data to be transmitted by the CCSDS 132 encoder is wrapped in a TM transfer frame, which encapsulates the data and provides the decoding side of CCSDS 132 with the necessary information to send the frame to the correct master and virtual channels.
In the current implementation, a TM transfer frame is 2,046 bytes in size. This includes the header and any trailers. When no trailer or secondary header is present, the data field is 2,040 octets in size. Since each TM transfer frame can only carry data from one virtual and master channel combination, each combination requires its own full-size buffer. The advantage of multiple buffers is that different virtual and master channels can send data to the CCSDS 132 encoder without being bottlenecked by other channels. Once a TM transfer frame is full, the data can be sent via the physical channel.
\autoref{fig:132Encoder} shows a visualisation of the architecture.

 \begin{figure}[hbt]
 	\centering
 	\includegraphics[width=\linewidth]{img/132_encoder.drawio.png}
 	\caption{Architecture of the 132 Encoder}
 	\label{fig:132Encoder}
 \end{figure}
...
Test Equipment is equipment not required by any of the system's requirements, but rather necessary for some of the tests described in the Verifcation- and Validation Plan.